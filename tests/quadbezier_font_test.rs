...
#[allow(dead_code)]
const AT: TextDescriptor<1> = TextDescriptor::<1> {
...
fn quadbezier_font_test() {
...

    {
        ...
        println!("Hash for `{}`: {}", "quadbezier_font_test", hash);

        for (ref_id, ref_hash) in common::REFERENCE_HASHES {
            if ref_id == "quadbezier_font_test" {
                hash_found = true;
